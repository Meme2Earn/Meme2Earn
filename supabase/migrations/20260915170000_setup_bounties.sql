-- Meme2Earn bounty campaign persistence
-- Run this after the profile setup migration.
...
with check (
  ...
  and reward > 0
  and poster <> ''
  and coin in ('USDG', 'PONS', 'CASHCAT', 'ARTIFICIAL_INU', 'NVDA', 'AAPL', 'MSFT')
);
...
create policy "Clients can create bounty joins"
on public.bounty_joins
for insert
to anon, authenticated
with check (bounty_id <> '' and wallet_address <> '');
...
create policy "Joined clients can create submissions"
on public.bounty_submissions
for insert
to anon, authenticated
with check (
  ...
  and exists (
    ...
    where bounty_joins.bounty_id = bounty_submissions.bounty_id
      and bounty_joins.wallet_address = bounty_submissions.wallet_address
  )
);
...
grant execute on function public.join_bounty(text, text, text, text) to anon, authenticated;
